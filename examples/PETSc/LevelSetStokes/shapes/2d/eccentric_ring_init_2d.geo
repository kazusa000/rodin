...
Disk(2) = {0.5, 0.5, 0, 0.19, 0.16};
Disk(3) = {0.56, 0.48, 0, 0.10, 0.07};

ring() = BooleanDifference{ Surface{2}; Delete; }{ Surface{3}; Delete; };
v() = BooleanFragments{ Surface{1}; Delete; }{ Surface{ring()}; Delete; };

eps = 1e-6;
obstacle[] = Surface In BoundingBox{0.29, 0.31, -eps, 0.71, 0.69, eps};
fluid[] = Surface{:};
fluid() -= obstacle();

interface[] = Curve In BoundingBox{0.29 - eps, 0.31 - eps, -eps, 0.71 + eps, 0.69 + eps, eps};
