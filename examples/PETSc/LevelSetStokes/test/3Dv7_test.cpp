/*
 * 3Dv7_test: 3Dv6 null-space gradient flow with
 ...
 *   - PETSc ILU nonzero shift for zero-pivot robustness
 *
 * This is intentionally a small, test-only stability variant of 3Dv6. It does
 * not use the old v7 rollback smoothing path.
 */
...
static constexpr const char* algorithmName = "3Dv7";
...
static void saveAndCleanSamePhaseInternalFaces(
  MeshType& mesh,
...
int main(int argc, char** argv)
{
  ...
  try
  {
    ...
    saveAndCleanSamePhaseInternalFaces(th, "Omega0.mesh", "Omega0.mesh");
    ...
    for (size_t it = 0; it < maxIt; ++it)
    {
      ...
      try
      {
        MMG::Optimizer()
          ...
          .optimize(th);
        clearSamePhaseInternalFaces(th, "after optimizer", true);
      ...
      try
      {
        ...
        clearSamePhaseInternalFaces(th, "after remesh", true);
        saveAndCleanSamePhaseInternalFaces(
          th,
