...
static constexpr Real defaultJLineSearchRtol = 5e-3;
static constexpr const char* algorithmName = "3Dv10_test";
  ...
  try
  {
    ...
    for (size_t it = 0; it < maxIt; ++it)
    {
      ...
      if (!currentQuality.accepted())
      {
        ...
        Alert::Warning()
          << "Current mesh rejected by quality gate at iteration " << it
      ...
      const Real jLineSearchTolerance =
        jLineSearchRtol * std::max<Real>(1.0, std::abs(static_cast<Real>(J)));
      ...
      for (size_t attempt = 0; attempt < maxRemeshAttempts; ++attempt)
      {
        ...
        try
        {
          ...
          if (!evalQuality.accepted())
          {
            Alert::Warning()
              << "Eval candidate rejected by quality gate after optimizer at iteration " << it
              << ", attempt=" << attempt
              << Alert::Raise;
            fLineSearch
              << it << " " << attempt << " " << attemptStepK << " "
              << attemptHMax << " " << attemptHMin << " " << attemptHausd << " "
              << "eval_quality_reject nan nan " << Jraw << " " << J << " "
              << jLineSearchTolerance << " "
              << remeshQuality.minTetVolume << " " << remeshQuality.minAltitudeRatio << " "
              << remeshQuality.badVolumeCells << " " << remeshQuality.badAltitudeCells << "\n";
            fLineSearch.flush();
          ...
          const bool objectiveAccepted = candidateObjective.J <= J + jLineSearchTolerance;
          Alert::Info()
