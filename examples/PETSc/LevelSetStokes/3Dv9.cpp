#define LEVELSET_STOKES_V9_STABLE_AREA 1

/*
 * 3Dv9: experimental damped active-set variant of 3Dv6 with
 ...
 * rotational semantics of 3Dv5. The surface-area inequality uses the dual
 * multiplier sign to decide whether it enters the null-space projection, and
 * active-set chatter reduces the level-set time step.
 */
...
#ifdef LEVELSET_STOKES_V9_STABLE_AREA
static constexpr bool stableAreaActiveSet = true;
static constexpr const char* algorithmName = "3Dv9";
#else
static constexpr bool stableAreaActiveSet = false;
...
int main(int argc, char** argv)
{
  ...
  try
  {
    ...
    Alert::Info()
      ...
      << ", stableAreaActiveSet=" << stableAreaActiveSet
    ...
    for (size_t it = 0; it < maxIt; ++it)
    {
...
#ifdef LEVELSET_STOKES_V9_STABLE_AREA
      if (areaViolation >= 0.0)
      {
        areaEverViolated = true;
      }
#endif
      ...
      const bool gramDegenerate =
        ...
        (stableAreaActiveSet && determinantRelative <= areaGramRelTol);
      Real areaCorrectionViolation = areaViolation;
      bool areaRangeActive = areaActive;
      bool areaNullActive = areaActive && !gramDegenerate;

#ifdef LEVELSET_STOKES_V9_STABLE_AREA
      // v9 stable active-set mode:
      //   - the range step corrects only positive area violations;
      //   - the null step follows the paper's dual-multiplier test below.
      ...
      const bool wantsAreaSystem = areaRangeActive || areaActive;
      bool useAreaSystem = wantsAreaSystem && !gramDegenerate;

#ifdef LEVELSET_STOKES_V9_STABLE_AREA
      if (wantsAreaSystem && gramDegenerate)
      {
      ...
      xiC = gradV;
      xiC *= rangeCoeffV;
      if (wantsAreaSystem && useAreaSystem)
      {
        GridFunction tmp(vh);
        tmp = gradA;
        tmp *= rangeCoeffA;
...
#ifdef LEVELSET_STOKES_V9_STABLE_AREA
      if (activeSetDampingEnabled && areaDebugMode == AreaDebugMode::Off)
      {
...
#ifdef LEVELSET_STOKES_V9_STABLE_AREA
        << ", areaEverViolated=" << areaEverViolated
#endif
...
#ifdef LEVELSET_STOKES_V9_STABLE_AREA
      if (wantsAreaSystem && gramRegularized && useAreaSystem)
      {
        Alert::Warning()
          << "   | area constraint Gram regularized: det(G)=" << determinant
          << ", detReg=" << solveDeterminant
          << ", ridge=" << gramRidge
          << ", detRel=" << determinantRelative
          << ", relThreshold=" << areaGramRelTol
          << Alert::Raise;
      }
#endif
