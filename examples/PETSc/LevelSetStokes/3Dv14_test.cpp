/*
 * 3Dv13: merged LevelSetStokes 3D variant.
 ...
 *   - selectable candidate acceptance:
 *       ACCEPT_POLICY=mesh_only -> V8/V9 remesh-quality acceptance
 *       ACCEPT_POLICY=objective -> V11 objective line-search acceptance
 */
...
static constexpr const char* defaultAcceptPolicy = "objective";
...
static constexpr Real defaultJLineSearchRtol = 5e-2;
...
static constexpr const char* algorithmName = "3Dv14_test";
...
static AcceptPolicy parseAcceptPolicy(const char* value)
{
  const std::string policy = value ? value : defaultAcceptPolicy;
  if (policy == "mesh_only" || policy == "MESH_ONLY" || policy == "mesh" || policy == "MeshOnly")
    return AcceptPolicy::MeshOnly;
  if (policy == "objective" || policy == "OBJECTIVE" || policy == "Objective")
    return AcceptPolicy::Objective;

  throw std::runtime_error("Unsupported ACCEPT_POLICY. Use mesh_only or objective.");
}
...
static const char* toString(AcceptPolicy policy)
{
  return policy == AcceptPolicy::Objective ? "objective" : "mesh_only";
}
...
int main(int argc, char** argv)
{
  ...
  try
  {
    ...
    const AcceptPolicy acceptPolicy =
      parseAcceptPolicy(LevelSetStokes::Runtime::envCString("ACCEPT_POLICY", defaultAcceptPolicy));
    ...
    Alert::Info()
      ...
      << ", acceptPolicy=" << toString(acceptPolicy)
    ...
    for (size_t it = 0; it < maxIt; ++it)
    {
      ...
      const bool gramDegenerate =
        determinant <= determinantThreshold ||
        (stableAreaPolicy && determinantRelative <= areaGramRelTol);
      ...
      bool useAreaSystem = wantsAreaSystem && !gramDegenerate;

      if (stableAreaPolicy && wantsAreaSystem && gramDegenerate)
      {
        gramRegularized = true;
        gramRidge =
          areaGramRelTol *
          std::max<Real>(Real(0.5) * (std::abs(innerVV) + std::abs(innerAA)), Real(1e-12));
        solveVV += gramRidge;
        solveAA += gramRidge;
        solveDeterminant = solveVV * solveAA - solveVA * solveVA;
        useAreaSystem = solveDeterminant > determinantThreshold;
        areaNullActive = false;
      }
      ...
      if (useAreaSystem)
      {
        ...
        if (stableAreaPolicy)
          areaNullActive = areaActive && useAreaSystem && areaDualMu > areaMultiplierTolerance;
      ...
      {
        if (havePreviousAreaState)
        {
          const bool nullStateChanged = areaNullActive != previousAreaNullActive;
          const bool crossedAreaBound = areaViolation * previousAreaViolation < 0.0;
          activeSetChattered = nullStateChanged || crossedAreaBound;
      ...
      for (size_t attempt = 0; attempt < maxRemeshAttempts; ++attempt)
      {
        ...
        try
        {
          ...
          if (acceptPolicy == AcceptPolicy::MeshOnly)
          {
          ...
          acceptedMesh = std::move(candidateEval);
      ...
      Alert::Info()
        << "   | Accepted " << toString(acceptPolicy) << " attempt=" << acceptedAttempt
