/*
 * 3Dv15: merged LevelSetStokes 3D variant.
 ...
 *   - selectable area policy:
 *       AREA_POLICY=basic  -> V8/V11 area active-set logic
 *       AREA_POLICY=stable -> V9 damped dual-multiplier active-set logic
 *   - candidate acceptance by remesh quality only; objective line-search removed
 *   - paper-style minimum-thickness penalty direction in the constraint system
 */
...
static constexpr Real   defaultAreaGramRelTol = 1e-3;
static constexpr Real   areaMultiplierTolerance = 1e-10;
static constexpr const char* defaultAreaPolicy = "stable";
static constexpr Real defaultActiveSetDampingDecay = 0.5;
...
static constexpr bool defaultAdaptSizeMapEnabled = true;
static constexpr Real defaultAdaptHNearFactor = 2.0;
static constexpr Real defaultAdaptHFarFactor = 1.0;
static constexpr Real defaultAdaptRNearFactor = 1.0;
static constexpr Real defaultAdaptRFarFactor = 0.5;
static constexpr Real defaultAdaptGradation = 1.3;
static constexpr const char* defaultOpenBlasNumThreads = "1";
static constexpr const char* defaultOmpNumThreads = "4";
static constexpr const char* algorithmName = "3Dv15_test";
...
struct MinThicknessStats
{
  Real penalty = 0.0;
  Real maxThinViolation = 0.0;
  size_t sampleCount = 0;
  size_t violatingSampleCount = 0;
  size_t skippedSampleCount = 0;
  std::vector<Real> rhs;
  std::vector<Real> nodalViolation;
};
...
static Vec3 operator/(const Vec3& a, Real s)
{
...
static Vec3 normalizedOrZero(const Vec3& a)
{
  const Real n = norm(a);
  if (n <= Real(0.0) || !std::isfinite(static_cast<double>(n)))
    return {};
  return a / n;
}

static Vec3 cellCentroid(const MeshType& mesh, Index cell)
{
  const auto& vertices = mesh.getConnectivity().getPolytope(3, cell);
  Vec3 c;
  for (auto i = decltype(vertices.size())(0); i < vertices.size(); ++i)
    c = c + toVec3(mesh.getVertexCoordinates(vertices[i]));
  return c / static_cast<Real>(vertices.size());
}

static bool orientGammaNormalFromObstacle(
  MeshType& mesh,
  Index face,
  Vec3& normal)
{
  auto& conn = mesh.getConnectivity();
  const auto& cells = conn.getIncidence({2, 3}, face);
  bool hasObstacleCell = false;
  Index obstacleCell = 0;
  for (auto i = decltype(cells.size())(0); i < cells.size(); ++i)
  {
    const auto attr = mesh.getAttribute(3, cells[i]);
    if (attr && *attr == Obstacle)
    {
      obstacleCell = cells[i];
      hasObstacleCell = true;
      break;
    }
  }
  if (!hasObstacleCell)
  ...
  const auto& vertices = mesh.getConnectivity().getPolytope(2, face);
  if (vertices.size() != 3)
    return false;
  ...
  const Vec3 centroid = (a + b + c) / Real(3.0);
  normal = normalizedOrZero(cross(b - a, c - a));
  if (norm(normal) <= Real(0.0))
    return false;

  const Vec3 out = centroid - cellCentroid(mesh, obstacleCell);
  if (dot(normal, out) < Real(0.0))
    normal = Real(-1.0) * normal;
  return true;
}

static bool barycentricTet(
  const Vec3& x,
  const Vec3& a,
  const Vec3& b,
  const Vec3& c,
  ...
  std::array<Real, 4>& lambda)
{
  ...
  const Vec3 xa = x - a;
  const Real r = dot(xa, cross(e2, e3)) / det;
  const Real s = dot(e1, cross(xa, e3)) / det;
  const Real t = dot(e1, cross(e2, xa)) / det;
  lambda = {Real(1.0) - r - s - t, r, s, t};
  return true;
}

static bool locateTetAndEvaluateDistance(
  const MeshType& mesh,
  const std::vector<Real>& distData,
  const Vec3& x,
  Real& distValue,
  Vec3& gradDist)
{
  ...
  for (auto cit = mesh.getCell(); cit; ++cit)
  {
    ...
    std::array<Real, 4> lambda;
    if (!barycentricTet(x, a, b, c, d, lambda))
      continue;

    bool inside = true;
    for (const Real l : lambda)
      inside = inside && l >= -tol && l <= Real(1.0) + tol;
    if (!inside)
      continue;

    distValue = 0.0;
    for (size_t i = 0; i < 4; ++i)
      distValue += lambda[i] * distData[static_cast<size_t>(vertices[i])];
    ...
    if (std::abs(det) <= Real(1e-30))
      return false;
    const Vec3 g1 = cross(e2, e3) / det;
    const Vec3 g2 = cross(e3, e1) / det;
    const Vec3 g3 = cross(e1, e2) / det;
    const Vec3 g0 = Real(-1.0) * (g1 + g2 + g3);
    gradDist =
        distData[static_cast<size_t>(vertices[0])] * g0
      + distData[static_cast<size_t>(vertices[1])] * g1
      + distData[static_cast<size_t>(vertices[2])] * g2
      + distData[static_cast<size_t>(vertices[3])] * g3;
    return true;
  }
  return false;
}

static Vec3 closestPointOnTriangle(
  const Vec3& p,
  const Vec3& a,
  const Vec3& b,
  const Vec3& c,
  std::array<Real, 3>& bary)
{
  ...
  bary = {Real(1.0) - v - w, v, w};
  return a + v * ab + w * ac;
}

static bool nearestGammaProjection(
  MeshType& mesh,
  const Vec3& x,
  std::array<Index, 3>& faceVertices,
  std::array<Real, 3>& bary,
  Vec3& normal)
{
  ...
  for (auto fit = mesh.getFace(); fit; ++fit)
  {
    ...
    std::array<Real, 3> candidateBary;
    const Vec3 a = toVec3(mesh.getVertexCoordinates(vertices[0]));
    const Vec3 b = toVec3(mesh.getVertexCoordinates(vertices[1]));
    const Vec3 c = toVec3(mesh.getVertexCoordinates(vertices[2]));
    const Vec3 closest = closestPointOnTriangle(x, a, b, c, candidateBary);
    const Real d2 = dot(closest - x, closest - x);
    if (d2 >= bestD2)
      continue;

    Vec3 candidateNormal;
    if (!orientGammaNormalFromObstacle(mesh, fit->getIndex(), candidateNormal))
      continue;

    bestD2 = d2;
    faceVertices = {vertices[0], vertices[1], vertices[2]};
    bary = candidateBary;
...
static std::vector<unsigned char> makeOuterBoundaryVertexMask(
  const MeshType& mesh,
  const FlatSet<Attribute>& baseOuterBdr)
{
  std::vector<unsigned char> mask(mesh.getVertexCount(), 0);
  for (auto fit = mesh.getFace(); fit; ++fit)
  {
    const auto attr = fit->getAttribute();
    if (!attr || baseOuterBdr.find(*attr) == baseOuterBdr.end())
      continue;
    const auto& vertices = fit->getVertices();
    for (auto i = decltype(vertices.size())(0); i < vertices.size(); ++i)
      mask[static_cast<size_t>(vertices[i])] = 1;
  }
  return mask;
}

static MinThicknessStats computeMinThicknessStatsAndRhs(
  MeshType& mesh,
  const std::vector<Real>& distData,
  const std::vector<Real>& kappaData,
  Real minThickness,
  size_t samples,
  const std::vector<unsigned char>& fixedVertexMask)
{
  MinThicknessStats stats;
  const size_t vertexCount = mesh.getVertexCount();
  stats.rhs.assign(3 * vertexCount, 0.0);
  stats.nodalViolation.assign(vertexCount, 0.0);

  if (minThickness <= Real(0.0) || samples == 0)
    return stats;
  ...
  const Real dxi = minThickness / static_cast<Real>(samples);
  for (auto fit = mesh.getFace(); fit; ++fit)
  {
    const auto attr = fit->getAttribute();
    if (!attr || *attr != GammaShape)
    ...
    Vec3 normal;
    if (!orientGammaNormalFromObstacle(mesh, fit->getIndex(), normal))
      continue;
    ...
    if (area <= Real(0.0) || !std::isfinite(static_cast<double>(area)))
      continue;

    const Vec3 x = (a + b + c) / Real(3.0);
    const Real kappaFace =
      (kappaData[static_cast<size_t>(vertices[0])]
       + kappaData[static_cast<size_t>(vertices[1])]
       + kappaData[static_cast<size_t>(vertices[2])]) / Real(3.0);
    const Real meanCurvature = -kappaFace;

    for (size_t sample = 0; sample < samples; ++sample)
    {
      ++stats.sampleCount;
      const Real xi = (static_cast<Real>(sample) + Real(0.5)) * dxi;
      const Vec3 xm = x - xi * normal;

      Real distValue = 0.0;
      Vec3 gradDist;
      if (!locateTetAndEvaluateDistance(mesh, distData, xm, distValue, gradDist))
      {
        ++stats.skippedSampleCount;
        continue;
      }

      const Real q = std::max(-distValue, Real(0.0));
      stats.maxThinViolation = std::max(stats.maxThinViolation, q);
      for (auto i = decltype(vertices.size())(0); i < vertices.size(); ++i)
      {
        auto& nodal = stats.nodalViolation[static_cast<size_t>(vertices[i])];
        nodal = std::max(nodal, q);
      }

      if (q <= Real(0.0))
        continue;

      std::array<Index, 3> projectionVertices;
      std::array<Real, 3> projectionBary;
      Vec3 projectionNormal;
      if (!nearestGammaProjection(mesh, xm, projectionVertices, projectionBary, projectionNormal))
      {
        ++stats.skippedSampleCount;
        continue;
      }

      ++stats.violatingSampleCount;
      const Real weight = area * dxi;
      stats.penalty += weight * q * q;

      const Vec3 gradDO = Real(-1.0) * gradDist;
      const Real localScalar = meanCurvature * q * q + Real(2.0) * q * dot(gradDO, normal);
      for (auto i = decltype(vertices.size())(0); i < vertices.size(); ++i)
      {
        const size_t v = static_cast<size_t>(vertices[i]);
        if (v >= fixedVertexMask.size() || fixedVertexMask[v])
          continue;
        const Real coeff = weight * Real(1.0 / 3.0) * localScalar;
        stats.rhs[v] += coeff * normal.x;
        stats.rhs[v + vertexCount] += coeff * normal.y;
        stats.rhs[v + 2 * vertexCount] += coeff * normal.z;
      }

      for (size_t i = 0; i < 3; ++i)
      {
        const size_t v = static_cast<size_t>(projectionVertices[i]);
        if (v >= fixedVertexMask.size() || fixedVertexMask[v])
          continue;
        const Real coeff = weight * projectionBary[i] * Real(-2.0) * q;
        stats.rhs[v] += coeff * projectionNormal.x;
        stats.rhs[v + vertexCount] += coeff * projectionNormal.y;
        stats.rhs[v + 2 * vertexCount] += coeff * projectionNormal.z;
      }
    }
  }
...
static bool solveSmallLinearSystem(
  ...
  std::vector<Real>& x)
{
  ...
  for (size_t k = 0; k < n; ++k)
  {
    ...
    if (pivotAbs <= Real(1e-30) || !std::isfinite(static_cast<double>(pivotAbs)))
      return false;
    if (pivot != k)
    {
...
template <class GF>
static std::vector<Real> copyGridFunctionData(const GF& gf)
{
  const auto& data = gf.getData();
  std::vector<Real> values(static_cast<size_t>(data.size()));
  for (auto i = decltype(data.size())(0); i < data.size(); ++i)
    values[static_cast<size_t>(i)] = static_cast<Real>(data[i]);
  return values;
}

template <class GF>
static bool setGridFunctionData(GF& gf, const std::vector<Real>& values)
{
  auto& data = gf.getData();
  if (static_cast<size_t>(data.size()) != values.size())
    return false;
  for (auto i = decltype(data.size())(0); i < data.size(); ++i)
    data[i] = values[static_cast<size_t>(i)];
  return true;
}
...
static AreaPolicy parseAreaPolicy(const char* value)
{
  const std::string policy = value ? value : defaultAreaPolicy;
  if (policy == "basic" || policy == "BASIC" || policy == "Basic")
    return AreaPolicy::Basic;
  if (policy == "stable" || policy == "STABLE" || policy == "Stable")
    return AreaPolicy::Stable;

  throw std::runtime_error("Unsupported AREA_POLICY. Use basic or stable.");
}

static const char* toString(AreaPolicy policy)
{
  return policy == AreaPolicy::Stable ? "stable" : "basic";
}
...
{
  setDefaultEnv("OPENBLAS_NUM_THREADS", defaultOpenBlasNumThreads);
  setDefaultEnv("OMP_NUM_THREADS", defaultOmpNumThreads);
  ...
  try
  {
    ...
    const AreaPolicy areaPolicy =
      parseAreaPolicy(LevelSetStokes::Runtime::envCString("AREA_POLICY", defaultAreaPolicy));
    const Real areaCorrectionGain =
      LevelSetStokes::Runtime::envDouble("AREA_CORRECTION_GAIN", defaultAreaCorrectionGain);
    const Real areaGramRelTol =
      LevelSetStokes::Runtime::envDouble("AREA_GRAM_REL_TOL", defaultAreaGramRelTol);
    const bool activeSetDampingEnabled =
      LevelSetStokes::Runtime::envDouble("ACTIVE_SET_DAMPING", 1.0) != 0.0;
    const Real activeSetDampingDecay =
      std::clamp(
        LevelSetStokes::Runtime::envDouble("ACTIVE_SET_DAMPING_DECAY", defaultActiveSetDampingDecay),
        Real(0.0), Real(1.0));
    const Real activeSetDampingRecovery =
      std::max(
        Real(1.0),
        LevelSetStokes::Runtime::envDouble("ACTIVE_SET_DAMPING_RECOVERY", defaultActiveSetDampingRecovery));
    const Real activeSetDampingMin =
      std::clamp(
        LevelSetStokes::Runtime::envDouble("ACTIVE_SET_DAMPING_MIN", defaultActiveSetDampingMin),
        Real(0.0), Real(1.0));
    ...
    const bool adaptSizeMapEnabled =
      LevelSetStokes::Runtime::envInt("ADAPT_SIZE_MAP", defaultAdaptSizeMapEnabled ? 1 : 0) != 0;
    const Real adaptHNearFactor =
      LevelSetStokes::Runtime::envDouble("ADAPT_H_NEAR_FACTOR", defaultAdaptHNearFactor);
    const Real adaptHFarFactor =
      LevelSetStokes::Runtime::envDouble("ADAPT_H_FAR_FACTOR", defaultAdaptHFarFactor);
    const Real adaptRNearFactor =
      LevelSetStokes::Runtime::envDouble("ADAPT_R_NEAR_FACTOR", defaultAdaptRNearFactor);
    const Real adaptRFarFactor =
      LevelSetStokes::Runtime::envDouble("ADAPT_R_FAR_FACTOR", defaultAdaptRFarFactor);
    ...
    const Real minThickness =
      LevelSetStokes::Runtime::envDouble("MIN_THICKNESS", Real(2.0) * hmin);
    const size_t minThicknessSamples =
      LevelSetStokes::Runtime::envSizeT("MIN_THICKNESS_SAMPLES", 4);
    const Real minThicknessActiveTol =
      LevelSetStokes::Runtime::envDouble("MIN_THICKNESS_ACTIVE_TOL", Real(0.25) * hmin);
    const Real minThicknessInactiveTol =
      LevelSetStokes::Runtime::envDouble("MIN_THICKNESS_INACTIVE_TOL", Real(0.10) * hmin);
    const Real minThicknessCorrectionGain =
      LevelSetStokes::Runtime::envDouble("MIN_THICKNESS_CORRECTION_GAIN", areaCorrectionGain);
    const Real minThicknessDualTol =
      LevelSetStokes::Runtime::envDouble("MIN_THICKNESS_DUAL_TOL", areaMultiplierTolerance);
    ...
    std::ofstream fThin("thin.txt");
    std::ofstream fCentroid("centroid.txt");

    if (!fObj || !fObjRaw || !fObjBefore || !fVol || !fNS || !fThin || !fCentroid)
    ...
    bool havePreviousAreaState = false;
    bool previousAreaNullActive = false;
    double previousAreaViolation = 0.0;
    bool havePreviousThinState = false;
    bool thinActiveState = false;
    bool previousThinNullActive = false;
    double previousThinViolation = 0.0;
    Real activeSetDamping = 1.0;
    ...
    Alert::Info()
      ...
      << ", areaPolicy=" << toString(areaPolicy)
      << ", surfaceAreaFactor=" << surfaceAreaFactor
      << ", areaCorrectionGain=" << areaCorrectionGain
      << ", areaGramRelTol=" << areaGramRelTol
      << ", minThickness=" << minThickness
      << ", minThicknessSamples=" << minThicknessSamples
      << ", minThicknessActiveTol=" << minThicknessActiveTol
      << ", minThicknessInactiveTol=" << minThicknessInactiveTol
      << ", minThicknessCorrectionGain=" << minThicknessCorrectionGain
      << ", minThicknessDualTol=" << minThicknessDualTol
      << ", activeSetDamping=" << activeSetDampingEnabled
      << ", activeSetDampingDecay=" << activeSetDampingDecay
      << ", activeSetDampingRecovery=" << activeSetDampingRecovery
      << ", activeSetDampingMin=" << activeSetDampingMin
      << ", kspType=" << kspType
      << ", kspPcType=" << kspPcType
      ...
      << ", adaptSizeMap=" << adaptSizeMapEnabled
      << ", adaptHNearFactor=" << adaptHNearFactor
      << ", adaptHFarFactor=" << adaptHFarFactor
      << ", adaptRNearFactor=" << adaptRNearFactor
      << ", adaptRFarFactor=" << adaptRFarFactor
      << ", adaptGradation=" << adaptGradation
      << ", OPENBLAS_NUM_THREADS=" << (std::getenv("OPENBLAS_NUM_THREADS") ? std::getenv("OPENBLAS_NUM_THREADS") : "")
      << ", OMP_NUM_THREADS=" << (std::getenv("OMP_NUM_THREADS") ? std::getenv("OMP_NUM_THREADS") : "")
    ...
    for (size_t it = 0; it < maxIt; ++it)
    {
      ...
      logMeshQuality(
        currentQuality,
        "after iteration input optimizer",
      ...
      Alert::Info() << "   | Computing minimum-thickness nonlocal RHS." << Alert::Raise;
      const auto fixedVertexMask = makeOuterBoundaryVertexMask(th, baseOuterBdr);
      const MinThicknessStats thinStats =
        computeMinThicknessStatsAndRhs(
          th,
          copyGridFunctionData(dist),
          copyGridFunctionData(kappa),
          minThickness,
          minThicknessSamples,
          fixedVertexMask);

      if (thinStats.maxThinViolation > minThicknessActiveTol)
        thinActiveState = true;
      else if (thinStats.maxThinViolation < minThicknessInactiveTol)
        thinActiveState = false;

      GridFunction thinViolation(sh);
      thinViolation = 0.0;
      {
        auto& data = thinViolation.getData();
        const size_t count = std::min(
          static_cast<size_t>(data.size()),
          thinStats.nodalViolation.size());
        for (size_t i = 0; i < count; ++i)
          data[i] = thinStats.nodalViolation[i];
      }
      ...
      // gradT in V: <gradT, psi>_V = DP_T(psi), assembled from the
      // nonlocal minimum-thickness derivative.
      PETSc::Variational::TrialFunction gradTTrial(vh); gradTTrial.setName("gradT");
      PETSc::Variational::TestFunction gradTTest(vh);

      Problem identifyT(gradTTrial, gradTTest);
      identifyT =
          Integral(alpha * alpha * Jacobian(gradTTrial), Jacobian(gradTTest))
        + Integral(gradTTrial, gradTTest)
        + DirichletBC(gradTTrial, VectorFunction{0.0, 0.0, 0.0}).on(baseOuterBdr);
      identifyT.assemble();
      {
        auto& rhs = identifyT.getLinearSystem().getVector();
        PetscErrorCode ierr = VecZeroEntries(rhs);
        if (ierr != PETSC_SUCCESS)
          throw std::runtime_error("Failed to zero MinT RHS.");
        for (size_t i = 0; i < thinStats.rhs.size(); ++i)
        {
          const Real value = thinStats.rhs[i];
          if (value == Real(0.0))
            continue;
          const PetscInt row = static_cast<PetscInt>(i);
          ierr = VecSetValue(rhs, row, value, ADD_VALUES);
          if (ierr != PETSC_SUCCESS)
            throw std::runtime_error("Failed to set MinT RHS value.");
        }
        ierr = VecAssemblyBegin(rhs);
        if (ierr != PETSC_SUCCESS)
          throw std::runtime_error("Failed to begin MinT RHS assembly.");
        ierr = VecAssemblyEnd(rhs);
        if (ierr != PETSC_SUCCESS)
          throw std::runtime_error("Failed to end MinT RHS assembly.");
      }
      ...
      GridFunction gradT(vh);
      gradT = gradTTrial.getSolution();

      const Real innerJV = innerVForm(gradJ, gradV);
      const Real innerJA = innerVForm(gradJ, gradA);
      const Real innerJT = innerVForm(gradJ, gradT);
      const Real innerVV = innerVForm(gradV, gradV);
      const Real innerVA = innerVForm(gradV, gradA);
      const Real innerVT = innerVForm(gradV, gradT);
      const Real innerAA = innerVForm(gradA, gradA);
      const Real innerAT = innerVForm(gradA, gradT);
      const Real innerTT = innerVForm(gradT, gradT);
      const bool stableAreaPolicy = areaPolicy == AreaPolicy::Stable;
      Real areaCorrectionViolation = areaViolation;
      bool areaRangeActive = areaActive;
      bool areaNullActive = false;

      if (stableAreaPolicy)
      {
        // Stable policy: only positive area violations enter the range step;
        // the null-space part is gated below by the dual multiplier sign.
        areaRangeActive = areaViolation > 0.0;
        areaNullActive = false;
        areaCorrectionViolation =
          areaRangeActive ? areaCorrectionGain * areaViolation : 0.0;
      }

      Real gramRidge = 0.0;
      bool gramRegularized = false;
      const bool wantsAreaSystem =
        stableAreaPolicy ? (areaRangeActive || areaActive) : areaRangeActive;
      const bool thinActive = thinActiveState;
      const bool thinRangeActive = thinActive && thinStats.penalty > Real(0.0);
      const Real thinCorrectionViolation =
        thinRangeActive ? minThicknessCorrectionGain * thinStats.penalty : Real(0.0);

      using GFType = std::decay_t<decltype(gradJ)>;
      struct ConstraintBasisEntry
      {
        const char* name = "";
        const GFType* grad = nullptr;
        Real rhsJ = 0.0;
        Real correction = 0.0;
      };

      auto innerConstraint = [&](const GFType& a, const GFType& b) -> Real
      {
        return innerVForm(a, b);
      };

      auto solveConstraintBasis =
        [&](const std::vector<ConstraintBasisEntry>& basis,
            const std::vector<Real>& rhs,
            std::vector<Real>& coeffs,
            bool allowRidge) -> bool
        {
          const size_t nBasis = basis.size();
          if (nBasis == 0 || rhs.size() != nBasis)
            return false;

          std::vector<std::vector<Real>> gram(nBasis, std::vector<Real>(nBasis, 0.0));
          Real diagScale = 0.0;
          for (size_t i = 0; i < nBasis; ++i)
          {
            for (size_t j = 0; j < nBasis; ++j)
              gram[i][j] = innerConstraint(*basis[i].grad, *basis[j].grad);
            diagScale += std::abs(gram[i][i]);
          }

          if (solveSmallLinearSystem(gram, rhs, coeffs))
            return true;
          ...
          const Real ridge =
            areaGramRelTol * std::max(diagScale / static_cast<Real>(nBasis), Real(1e-12));
          for (size_t i = 0; i < nBasis; ++i)
            gram[i][i] += ridge;
          if (!solveSmallLinearSystem(gram, rhs, coeffs))
            return false;

          gramRegularized = true;
          gramRidge = std::max(gramRidge, ridge);
          return true;
        };

      std::vector<ConstraintBasisEntry> trialBasis{
        {"V", &gradV, innerJV, volumeViolation}
      };
      if (wantsAreaSystem)
        trialBasis.push_back({"A", &gradA, innerJA, areaCorrectionViolation});
      if (thinActive)
        trialBasis.push_back({"T", &gradT, innerJT, thinCorrectionViolation});

      std::vector<Real> trialRhs;
      trialRhs.reserve(trialBasis.size());
      for (const auto& entry : trialBasis)
        trialRhs.push_back(entry.rhsJ);

      std::vector<Real> trialProjCoeffs;
      const bool useTrialSystem =
        solveConstraintBasis(trialBasis, trialRhs, trialProjCoeffs, stableAreaPolicy || thinActive);

      Real trialProjCoeffV = 0.0;
      Real trialProjCoeffA = 0.0;
      Real trialProjCoeffT = 0.0;
      Real areaDualMu = 0.0;
      Real thinDualMu = 0.0;
      if (useTrialSystem)
      {
        for (size_t i = 0; i < trialBasis.size(); ++i)
        {
          if (std::string(trialBasis[i].name) == "V")
            trialProjCoeffV = trialProjCoeffs[i];
          else if (std::string(trialBasis[i].name) == "A")
            trialProjCoeffA = trialProjCoeffs[i];
          else if (std::string(trialBasis[i].name) == "T")
            trialProjCoeffT = trialProjCoeffs[i];
        }
        areaDualMu = -trialProjCoeffA;
        thinDualMu = -trialProjCoeffT;
      }

      if (stableAreaPolicy)
        areaNullActive = areaActive && useTrialSystem && areaDualMu > areaMultiplierTolerance;
      else
        areaNullActive = areaActive && useTrialSystem;
      bool thinNullActive =
        thinActive && useTrialSystem && thinDualMu > minThicknessDualTol;
      ...
      Real rangeCoeffT = 0.0;

      std::vector<ConstraintBasisEntry> projectionBasis{
        {"V", &gradV, innerJV, volumeViolation}
      };
      if (areaNullActive)
        projectionBasis.push_back({"A", &gradA, innerJA, areaCorrectionViolation});
      if (thinNullActive)
        projectionBasis.push_back({"T", &gradT, innerJT, thinCorrectionViolation});

      std::vector<Real> projectionRhs;
      projectionRhs.reserve(projectionBasis.size());
      for (const auto& entry : projectionBasis)
        projectionRhs.push_back(entry.rhsJ);
      std::vector<Real> projectionCoeffs;
      if (!solveConstraintBasis(projectionBasis, projectionRhs, projectionCoeffs, true))
      {
        projectionBasis = {{"V", &gradV, innerJV, volumeViolation}};
        projectionRhs = {innerJV};
        if (!solveConstraintBasis(projectionBasis, projectionRhs, projectionCoeffs, true))
          projectionCoeffs = {innerJV / std::max(innerVV, Real(1e-30))};
        areaNullActive = false;
        thinNullActive = false;
      }
      for (size_t i = 0; i < projectionBasis.size(); ++i)
      {
        if (std::string(projectionBasis[i].name) == "V")
          projCoeffV = projectionCoeffs[i];
        else if (std::string(projectionBasis[i].name) == "A")
          projCoeffA = projectionCoeffs[i];
        else if (std::string(projectionBasis[i].name) == "T")
          projCoeffT = projectionCoeffs[i];
      }

      std::vector<ConstraintBasisEntry> rangeBasis{
        {"V", &gradV, innerJV, volumeViolation}
      };
      if (areaRangeActive)
        rangeBasis.push_back({"A", &gradA, innerJA, areaCorrectionViolation});
      if (thinRangeActive)
        rangeBasis.push_back({"T", &gradT, innerJT, thinCorrectionViolation});

      std::vector<Real> rangeRhs;
      rangeRhs.reserve(rangeBasis.size());
      for (const auto& entry : rangeBasis)
        rangeRhs.push_back(entry.correction);
      std::vector<Real> rangeCoeffs;
      const bool useRangeSystem = solveConstraintBasis(rangeBasis, rangeRhs, rangeCoeffs, true);
      if (!useRangeSystem)
      {
        rangeBasis = {{"V", &gradV, innerJV, volumeViolation}};
        rangeCoeffs = {volumeViolation / std::max(innerVV, Real(1e-30))};
      }
      for (size_t i = 0; i < rangeBasis.size(); ++i)
      {
        if (std::string(rangeBasis[i].name) == "V")
          rangeCoeffV = rangeCoeffs[i];
        else if (std::string(rangeBasis[i].name) == "A")
          rangeCoeffA = rangeCoeffs[i];
        else if (std::string(rangeBasis[i].name) == "T")
          rangeCoeffT = rangeCoeffs[i];
      }

      const size_t activeBasisDim = trialBasis.size();
      ...
      GridFunction normGradT(shNormSpace);
      normGradT = Frobenius(gradT);
      ...
      const Real maxGradT = normGradT.max();
      ...
      const Real thinOrthResidual = innerVForm(xiJ, gradT);
      Real effectiveStepK = stepK;
      bool activeSetChattered = false;

      if (stableAreaPolicy && activeSetDampingEnabled && areaDebugMode == AreaDebugMode::Off)
      {
        if (havePreviousAreaState || havePreviousThinState)
        {
          const bool areaNullStateChanged =
            havePreviousAreaState && areaNullActive != previousAreaNullActive;
          const bool crossedAreaBound =
            havePreviousAreaState && areaViolation * previousAreaViolation < 0.0;
          const bool thinNullStateChanged =
            havePreviousThinState && thinNullActive != previousThinNullActive;
          const bool crossedThinActive =
            havePreviousThinState &&
            ((previousThinViolation <= minThicknessActiveTol && thinStats.maxThinViolation > minThicknessActiveTol) ||
             (previousThinViolation >= minThicknessInactiveTol && thinStats.maxThinViolation < minThicknessInactiveTol));
          activeSetChattered =
            areaNullStateChanged || crossedAreaBound || thinNullStateChanged || crossedThinActive;

          if (activeSetChattered)
          {
            activeSetDamping =
              std::max(activeSetDampingMin, activeSetDamping * activeSetDampingDecay);
          }
          else
          {
            activeSetDamping =
              std::min(Real(1.0), activeSetDamping * activeSetDampingRecovery);
          }
        }

        havePreviousAreaState = true;
        previousAreaNullActive = areaNullActive;
        previousAreaViolation = areaViolation;
        havePreviousThinState = true;
        previousThinNullActive = thinNullActive;
        previousThinViolation = thinStats.maxThinViolation;
        effectiveStepK = stepK * activeSetDamping;
      }
      ...
      Alert::Info()
        ...
        << ", activeBasisDim=" << activeBasisDim
        << ", areaActive=" << areaActive
        << ", areaRangeActive=" << areaRangeActive
        << ", areaNullActive=" << areaNullActive
        << ", thinPenalty=" << thinStats.penalty
        << ", maxThinViolation=" << thinStats.maxThinViolation
        << ", thinActive=" << thinActive
        << ", thinRangeActive=" << thinRangeActive
        << ", thinNullActive=" << thinNullActive
        << ", stableAreaPolicy=" << stableAreaPolicy
        << ", gramRegularized=" << gramRegularized
        << ", gramRidge=" << gramRidge
        << ", areaDualMu=" << areaDualMu
        << ", thinDualMu=" << thinDualMu
        << ", areaCorrection=" << areaCorrectionViolation
        << ", thinCorrection=" << thinCorrectionViolation
        << ", activeSetChattered=" << activeSetChattered
        << ", activeSetDamping=" << activeSetDamping
        << ", effectiveStepK=" << effectiveStepK
        << ", max|gradJ|=" << maxGradJ
        << ", max|gradV|=" << maxGradV
        << ", max|gradA|=" << maxGradA
        << ", max|gradT|=" << maxGradT
      ...
      if ((areaRangeActive || thinRangeActive) && !useRangeSystem)
        Alert::Warning()
          << "   | constraint range correction fallback to volume-only."
          << Alert::Raise;

      Alert::Info()
        << "   | Range space diag: projCoeffV=" << projCoeffV
        << ", projCoeffA=" << projCoeffA
        << ", projCoeffT=" << projCoeffT
        << ", rangeCoeffV=" << rangeCoeffV
        << ", rangeCoeffA=" << rangeCoeffA
        << ", rangeCoeffT=" << rangeCoeffT
        << ", max|xiC|=" << maxXiC
        << Alert::Raise;
      ...
      if (areaDebugMode == AreaDebugMode::Off)
      {
        ...
        Alert::Info()
          << "  alphaJ=" << alphaJ
          << ", alphaC=" << alphaC
          << ", stepK=" << effectiveStepK
          << ", dispJ=" << effectiveStepK * alphaJ * maxXiJ
          << ", dispC=" << effectiveStepK * alphaC * maxXiC
          << Alert::Raise;
      }
      else
      {
        ...
        Alert::Warning()
          ...
          << ", dispA=" << effectiveStepK * areaDebugAlpha * maxGradA
          << Alert::Raise;
      }
      ...
      Advection::Lagrangian(advect, test, dist, thetaField).step(effectiveStepK);
      ...
      domainGrid.add("thinViolation", thinViolation, IO::XDMF::Center::Node);
      ...
      for (size_t attempt = 0; attempt < maxRemeshAttempts; ++attempt)
      {
        ...
        Real attemptStepK = effectiveStepK;
        if (attempt == 1)
        {
          attemptStepK = Real(0.5) * effectiveStepK;
        }
        else if (attempt == 2)
        {
          attemptStepK = Real(0.25) * effectiveStepK;
        }
        else if (attempt == 3)
        {
          attemptStepK = Real(0.5) * effectiveStepK;
        ...
        {
          attemptStepK =
            std::max(Real(0.05) * effectiveStepK, effectiveStepK / std::pow(Real(2.0), static_cast<Real>(attempt)));
        ...
        try
        {
          ...
          {
            {
              auto& conn = candidateRaw.getConnectivity();
              conn.compute(2, 3);
              conn.compute(3, 2);
              conn.compute(2, 1);
              conn.compute(1, 0);
            ...
            const Real adaptHNear =
              std::clamp(adaptHNearFactor * attemptHMin, attemptHMin, attemptHMax);
            const Real adaptHFar =
              std::max(adaptHNear, adaptHFarFactor * attemptHMax);
            const Real adaptRNear =
              std::max(Real(0.0), adaptRNearFactor * attemptHMin);
            const Real adaptRFar =
              std::max(adaptRNear + Real(1e-12), adaptRFarFactor * attemptHMax);
            const Real adaptWidth = std::max(adaptRFar - adaptRNear, Real(1e-12));
          ...
          acceptedObjective = ObjectiveSample{
            Jraw,
            J,
            obstacleVolume,
            shapeArea,
      ...
      fNS  << alphaJ << " " << alphaC << " "
           << activeBasisDim << " "
           << (areaActive ? 1 : 0) << " " << (areaNullActive ? 1 : 0) << " "
           << (thinActive ? 1 : 0) << " " << (thinNullActive ? 1 : 0) << " "
           << projCoeffV << " " << projCoeffA << " " << projCoeffT << " "
           << rangeCoeffV << " " << rangeCoeffA << " " << rangeCoeffT << " "
           << maxXiJ << " " << maxXiC << "\n";
      fThin << thinStats.penalty << " "
            << thinStats.maxThinViolation << " "
            << (thinActive ? 1 : 0) << " "
            << (thinNullActive ? 1 : 0) << " "
            << thinDualMu << " "
            << thinCorrectionViolation << " "
            << thinStats.sampleCount << " "
            << thinStats.violatingSampleCount << " "
            << thinStats.skippedSampleCount << "\n";
      fCentroid
      ...
      Alert::Info()
        << "   | Accepted mesh-quality attempt=" << acceptedAttempt
